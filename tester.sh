...
echo "-----Student Output Ends-----"
read -n 1 -p "i is an integer, used as an array. Error message related to that? (y/n)" mainmenuinput
if [ $mainmenuinput = "y" ]; then
	error=$(($error + 1))
fi
echo ""
...
echo "-----Student Output Ends-----"
read -n 1 -p "r not defined Error message related to that? (y/n)" mainmenuinput
if [ $mainmenuinput = "y" ]; then
	error=$(($error + 1))
fi
echo ""
...
echo "-----Student Output Ends-----"
read -n 1 -p "Not more input data. Error message related to that? (y/n)" mainmenuinput
if [ $mainmenuinput = "y" ]; then
	error=$(($error + 1))
fi
echo ""
